...
LPBYTE	DibLoadHandle( wchar_t *lpFileName )
{
	FILE				*fp;
	...
	if ( !(fp = _wfopen( lpFileName, L"rb" )) ) return NULL;
...
bool	DibSave( LPBYTE lpDib, wchar_t *lpFileName )
{
	FILE				*fp;
	BITMAPFILEHEADER	bmfh;

	if ( !lpDib || !lpFileName ) return false;
	if ( !(fp = _wfopen( lpFileName, L"wb" )) ) return false;
